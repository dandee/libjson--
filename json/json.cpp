...
Json::Json(const Json& json)
{
    ...
    for (std::map<std::string, Json*>::const_iterator i = json.children_.begin();
         i != json.children_.end();
         i++)
    {
    ...
    for (p++; *p && *p != '"'; p++)
    {
        if (*p == '\\')
        {
            ...
            case 'u':
            {
                ...
                for (p++, i = 0;
                     i < 4 &&
                     ((*p >= '0' && *p <= '9') || (tolower(*p) >= 'a' && tolower(*p) <= 'f'));
                     p++)
                {
...
{
    if ((type_ != Json::JTYPE_STRING) && (type_ != Json::JTYPE_BOOLEAN) &&
        (type_ != Json::JTYPE_NUMBER))
    {
        throw "Cannot convert object or array types"; // FIXME: should throw an appropriate
    }
